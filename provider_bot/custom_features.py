...
@register_entity_feature(feature_name='colon-feature')
def extract_colon_feature(**args):
    def _extractor(example, resources):
        query, entities, entity_index = example
        return {'has_colon': ':' in entities[entity_index].text}

    return _extractor

@register_entity_feature(feature_name='entity-length')
def extract_entity_length(**args):
    def _extractor(example, resources):
        query, entities, entity_index = example
        return {'entity_length': len(entities[entity_index].text)}
